...
/**
 * Customers assigned to currentUserId (Customer.responsibleId), scoped
 * exactly as before (Stage 8C): responsibleId + isActive: true as the
 * base scope, which a status filter narrows further but never bypasses —
 * an isActive:false customer never appears regardless of status.
 *
...
export async function listSalesCustomers(
  ...
  const { search, status, page: rawPage, limit = 20 } = options;
